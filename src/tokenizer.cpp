...
Tokenizer::AllTokens(std::string input)
{
  ...
  {
    readLine(&f);
    std::cout << "No " << ++cnt << " line" << std::endl;
    detect_comment = false;
    while (true)
    {
      auto t = nextToken();
      if(detect_comment) // 检测到注释
        break;
      // 加一个判断取到的token是否正确, pair
      if (t.second.has_value())
      {
        if (t.second.value().getCode() != ErrorCode::ErrEOF)
        {
          return std::make_pair(std::vector<Token>(), t.second);
        }
        else{ // 一行读入完成了
          printf("i get the eol\n");
          break;
        }
      }
      else{
        result.emplace_back(t.first.value());
